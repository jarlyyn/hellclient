...
也可以在打开网页界面后，点击界面最上方的修改密码按钮修改密码

![修改密码按钮](images/password.png)
...
linux版本的启动程序为/bin/hellclient

### 安全组

Hellclient的设计目标是在vps上长期运行脚本，然后通过网页界面或者客户端(HellclientUI)，通过电脑或者手机进行控制。所以，当你在云服务器上使用时，需要在云服务商的网页后台，在服务器对应的云防火墙(一般较安全组)中，打开所有ip的tcp 4355端口访问

* [阿里云安全组设置说明](https://developer.aliyun.com/article/767328)

* [腾讯云安全组设置说明](https://cloud.tencent.com/document/product/213/112614)
...
**注意，右边的，浅橙色的“编辑变量说明”是改脚本，改变量的按钮是左边的“变量”**
...
* 游戏ID,说明当前是哪个游戏
* **小人按钮，助理图标，需要运行的脚本支持，可以快速调用功能，类似于Windows的开始菜单**
* 输入框，输入并发送文字到mud
* 省略号的历史输入信息
...
* /appdata/game/worlds 游戏文件,每个游戏以toml格式储存，可以直接复制重命名作为新id打开
* /appdata/game/scripts 脚本目录
* /appdata/game/logs 日志目录
* /appdata/game/shared 共享数据目录

在备份或升级客户端时，只要备份appdata/configs目录即可

### 线报服务器

在config/system.toml中有一行hellswitch是线报服务器的设置。

如果正确的连接到了线报服务器，在页面右上角会有个绿色的连接标志

![](./images/switch.png)

有这个标志的情况下，你就能和所有连接到这个交换器的客户端交换信息了

### 使用客户端

网页模式一般仅作为正常运行的测试使用

平时使用，建议使用HellclientUI管理终端。
